...
export default function Home() {
  ...
  const fetchGitHubData = async () => {
    ...
    try {
      ...
      if (!records.includes(username)) {
        records.push(username);
        localStorageUtils.setItem("searchRecords", records);
        setSearchRecords(records);
      }
      setUserData(data);
    } catch (err) {
      ...
      {userData && (
        <>
          ...
          <button
            ...
            className="bg-blue-200 flex gap-2 justify-center text-blue-500 font-bold text-center p-3 mt-6 rounded-sm shadow-md w-full max-w-md transition-all duration-300 ease-in-out transform hover:gap-3 hover:-rotate-2 hover:scale-105"
          >
